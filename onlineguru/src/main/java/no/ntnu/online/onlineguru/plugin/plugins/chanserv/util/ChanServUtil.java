...
public class ChanServUtil {
	...
	public ChanServUtil(WandRepository wandRepository, AuthorizedUserHandler userHandler) {
	...
	public boolean requireConditions(boolean hasAccess, boolean botHasOp, char flag, String channel, AuthorizedUser user, String sender, Network network) {
		...
		if(channel != null && !channel.isEmpty()) {
			if(!wandRepository.amIOnChannel(network, channel)) {
				wandRepository.sendMessageToTarget(network, sender, "I am not on that channel.");
				return false;
			}
		}
		if(hasAccess) {
			if(!user.isSuperuser()) {
				if(channel != null && !channel.isEmpty()) {
					if(!user.hasAccess(channel)) {
						wandRepository.sendMessageToTarget(network, sender, "You do not have access to that channel.");
						return false;
					} else {
						if(flag != 0) {
							if(!user.getFlags(channel).containsFlag(flag)) {
								wandRepository.sendMessageToTarget(network, sender, "You do not have the privileges to do that.");
								return false;
							}
						}
					}
		...
		if(botHasOp) {
			if(channel != null && !channel.isEmpty()) {
				if(!wandRepository.amIOp(network, channel)) {
					wandRepository.sendMessageToTarget(network, sender, "I am not an operator on that channel.");
					return false;
				}
			}
		}
		if(flag != 0) {
			...
			if(flags != null) {
				if(!flags.containsFlag(flag)) {
					wandRepository.sendMessageToTarget(network, sender, "You do not have the necessary privileges on that channel.");
					return false;
				}
			} else {
				if(!user.isSuperuser()) {
