...
public class DecodeHtmlEntities {
    ...
    private static final int LONGEST_ENTITY = "&thetasym;".length();
	...
	public static String stripHTMLEntities(String text) {
	    ...
	    for (int i=0; i<originalTextLength; i++) {
	        ...
	        if (whereAmp < 0) {
	            ...
	            String possibleEntity = text.substring(i + 1, Math.min(i + LONGEST_ENTITY, text.length()));
	            char t = translateEntity(possibleEntity);
	            
	            if (t != 0) {
	                // was a good entity, keep its equivalent char.
	                sb.append(t);
	                // avoid reprocessing chars forming the entity
	                int whereSemi = possibleEntity.indexOf(";", SHORTEST_ENTITY - 2);
	                i += whereSemi + 1;
                }
	            else {
	...
	private static char translateEntity(String possibleEntity) {
		// If it's shorter than the shortest entity, it cannot be translated
		// to a valid char.
        if (possibleEntity.length() < SHORTEST_ENTITY - 1) {
            return 0;
        }

        // Find the trailing ;
        // Start where the shortest entity would end
        int whereSemi = possibleEntity.indexOf(";", SHORTEST_ENTITY - 2);
	    ...
	    try {
	        if (bareEntity.charAt(0) == '#') {
	            ...
	            if (secondChar == 'x' || secondChar == 'X') {
	                ...
	                // had &_#x123D_; parse hex value to int then char
